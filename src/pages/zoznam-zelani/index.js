      ...
      <div className="max-w-4xl mx-auto p-4 md:p-6">
        <div className="flex flex-col gap-4">
          {products.map((product) => {
            ...
            const isInCart = cart?.items?.some((item) => item.variant_id === variant?.id);

            const effectiveAvailableQty = Math.max(stock - (isInCart ? 1 : 0), 0);
              ...
              >
                ...
                <div className="flex-1 w-full">
                  ...
                  {(!isRetiazka(product) ||
                    ...
                    <p className={`text-xs sm:text-sm ${effectiveAvailableQty > 0 ? "text-green-600" : "text-red-600"}`}>
                ...
                <div className="flex flex-col gap-2 items-stretch md:items-end w-full md:w-auto">
                  <div className="flex gap-2 items-stretch w-full flex-col">
                  {isRetiazka(product) && (
                    <div className="relative w-full md:w-auto">
                      <input
                        ...
                        value={
                          localStorage.getItem(`necklace-length-${product.handle}`)
                            ? `${localStorage.getItem(`necklace-length-${product.handle}`)}cm`
                            : ""
                        }
                        readOnly
                        onClick={() =>
                          setOpenMenuFor((prev) => (prev === product.id ? null : product.id))
                        }
